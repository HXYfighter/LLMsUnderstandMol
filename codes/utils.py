...
# @torch.no_grad()
def likelihood(model, smiles=None, tokenizer=None, vocab=None, seqs=None):
    nll_loss = nn.NLLLoss(reduction="none", ignore_index=0)
    if seqs == None:
        tokens = [tokenizer.tokenize(smile) for smile in smiles]
        encoded = [vocab.encode(token) for token in tokens]
        sequences = [torch.tensor(encode, dtype=torch.long) for encode in encoded]

        max_length = max([seq.size(0) for seq in sequences])
        seqs = torch.zeros(len(sequences), max_length, dtype=torch.long)
        ...
        for i in range(seqs.size(0)):
            for j in range(seqs.size(1)):
                if seqs[i, j].item() == 1:
                    seqs[i, (j + 1):] = torch.zeros(seqs.size(1) - j - 1, dtype=torch.long)
                    break

    seqs = seqs.cuda()
    # print(seqs.shape, seqs[-1])
