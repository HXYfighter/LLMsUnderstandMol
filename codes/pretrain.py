...
if __name__ == '__main__':
    ...
    parser.add_argument('--run_name', type=str, help="name for tensorboard run", default='chembl_pretrain1')
    ...
    parser.add_argument('--max_epochs', type=int, default=20, help="total epochs", required=False)
    parser.add_argument('--batch_size', type=int, default=4096, help="batch size", required=False)
    parser.add_argument('--learning_rate', type=float, default=5e-3, help="learning rate", required=False)
    ...
    parser.add_argument('--vocab_path', type=str, default="data/zinc/vocab.txt", required=False)
    parser.add_argument('--ckpt_load_path', type=str, default="ckpt/chembl_pretrain1/epoch9.pt", required=False)
    ...
    if args.ckpt_load_path != None:
        print("Load check point from " + args.ckpt_load_path)
    ...
    for epoch in tqdm(range(args.max_epochs)):
        ...
        for iter_num, (x, y) in pbar:
            ...
            if (iter_num + num_batches * epoch) % 100 == 0:
                ...
                val_losses = []
                with torch.no_grad():
                    for val_iter_num, (x, y) in enumerate(val_loader):
                        x = x.to(device)
                        y = y.to(device)
                        logits, loss = model(x, y)
                        loss = loss.mean()
                ...
                writer.add_scalar('validation loss', val_loss, iter_num + num_batches * epoch)
                model.train()

        # save checkpoint
        # torch.save(model.module.state_dict(), args.ckpt_save_path + args.run_name + "/" + f"epoch{epoch}.pt")
        torch.save(model.state_dict(), args.ckpt_save_path + args.run_name + "/" + f"epoch{epoch}.pt")
